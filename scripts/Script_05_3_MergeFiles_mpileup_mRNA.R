...
for(i in 1:length(cell.ids)) {
    ...
    file.in <- paste(cell.id, "_mpileup.txt", sep="")
...
names(df) <- c("cell.id", "chr", "start", "V3", "coverage", "base", "V6")
df <-  df[,c("chr", "start", "coverage", "base", "cell.id")]

# Save file
path <- "/project/meadlab/wwen/Vladimir/out/"
file <- "GST010_mpileup_mRNA.txt"
